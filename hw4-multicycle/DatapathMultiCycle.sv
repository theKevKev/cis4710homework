...
`endif
  ...
  // counters (program, insn, cycle)
  logic [`REG_SIZE] pcNext, pcCurrent;
  logic [`REG_SIZE] cycles_current, num_insns_current;
  logic [3:0] div_cycles_current;
  always @(posedge clk) begin
    if (rst) begin
      div_cycles_current <= 0;
      cycles_current <= 0;
      pcCurrent <= 32'd0;
      num_insns_current <= 0;
    end else begin
      div_cycles_current <= (is_div_cycle && div_cycles_current < `N_DIVIDER_STAGES) ? div_cycles_current + 1 : 0;
      cycles_current <= cycles_current + 1;
      if (!is_div_cycle || div_cycles_current == `N_DIVIDER_STAGES) begin
        pcCurrent <= pcNext;
  ...
  assign trace_completed_cycle_status = (is_div_cycle && div_cycles_current < `N_DIVIDER_STAGES) ? CYCLE_DIV : CYCLE_NO_STALL;
